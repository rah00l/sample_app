...
class ReportPdf < Prawn::Document
  def initialize(products)
    super()
    # @products = products
    @products = [{" Department Totals 108%"=>
   {"Regular Hours"=>["", "13605.00"],
    "99999 Colored Dot Label"=>["", "14"]
   }
 },
 {"Andria Snyder 99%"=>
   {"Regular Hours"=>["", "563.00"],
    "Marc Record (diskette)"=>["", "41"],
    "Marc Record Ipage/ftp"=>["", "1293"],
    "9999999 - K3 Auto W/pockt-cat"=>["", "31"]
  }
},
 {"Angela Dehart ?"=>
   {"Cartons"=>["", "17"],
  ...
  def product_rows
    ...
    @products.each do |item|
      item.each do |k,v|
        v.each_with_index do |(k1,v1), i|
          # binding.pry if i%29==0
          if i%29==0
            arr << ['Associate','Process Function','Avg','Total']
            arr << [{content: k, rowspan: 29},k1,v1[0],v1[1]]
          else
            arr << [k1,v1[0],v1[1]]
          end
        end
